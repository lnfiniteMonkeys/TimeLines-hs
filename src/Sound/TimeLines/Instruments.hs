...
import Sound.TimeLines.Expression
...
flattenLists :: [[SigExpr]] -> SigExpr -> [SigExpr]
...
>>>>>>> master:src/Sound/TimeLines/Instruments.hs

-- | Speeds up a signal by an amount
speed :: SigExpr -> SigExpr -> SigExpr
speed s  = Signal $ \t -> sf $ (amt t)*t
fast = speed

-- | Offsets a signal in time by an amount
offset :: SigExpr -> SigExpr -> SigExpr
...
semi :: (Num a, Floating a, Eq a) => SigExpr -> SigExpr
semi s = 2**(s/12)
...
-- | (i.e. between 0 and 1)
fromList :: (RealFrac b) => [SigExpr] -> Signal b -> SigExpr
...
-- | Smoothstep
smoothstep :: SigExpr -> SigExpr -> SigExpr -> SigExpr
...
lerp :: (Num a) => SigExpr -> SigExpr -> SigExpr -> SigExpr
...
pow :: (Floating a, Eq a) => SigExpr -> SigExpr -> SigExpr
pow = flip (**)
...
{-
envR :: SigExpr -> SigExpr -> SigExpr -> SigExpr -> SigExpr -> SigExpr
...
{-
envlp :: (Floating a, Ord a) => SigExpr -> SigExpr -> SigExpr -> SigExpr -> SigExpr -> SigExpr
